...
(function (exports) {
  ...
  function formatElementDBForLLM(elementDB) {
    ...
    for (const type of typeOrder) {
      ...
      for (const el of group) {
        ...
        // Show pages (prefer Mendix form path over URL)
        if (el.pages?.length) {
          const pageStrs = el.pages.map(p => p.formPath || p.url || p.title).filter(Boolean);
          if (pageStrs.length) desc += ` — pages: ${pageStrs.join(', ')}`;
        }
   ...
   * @param {object} [context] — Optional context with { pageUrl } as starting page
   * @returns {object} — Updated element DB
   */
  function enrichFromSteps(elementDB, steps, context) {
    const db = elementDB || { elements: {} };
    if (!db.elements) db.elements = {};
    let currentPageUrl = context?.pageUrl || '';

    for (const step of steps) {
      // Track current page from Navigate steps
      if (step.action === 'Navigate' && step.value) {
        currentPageUrl = step.value;
        continue;
      }
      ...
      if (existing) {
        ...
        // Track page where element appears
        if (currentPageUrl) {
          if (!existing.pages) existing.pages = [];
          if (!existing.pages.some(p => p.url === currentPageUrl)) {
            existing.pages.push({ url: currentPageUrl, title: '' });
          }
        }
      } else {
        // New element
        ...
        db.elements[elementKey] = {
          ...
          selectors,
          context: currentPageUrl ? { pageUrl: currentPageUrl } : {},
          pages: currentPageUrl ? [{ url: currentPageUrl, title: '' }] : [],
