...
(function (exports) {
  ...
  function mergeElements(existingDB, discovered, context) {
    ...
    for (const el of discovered) {
      ...
      if (existing) {
        ...
        // Update context if this is a new page — prefer per-element pageUrl
        const elPageUrl = el.pageUrl || context?.pageUrl;
        const elPageTitle = el.pageTitle || context?.pageTitle || '';
        if (elPageUrl && existing.context?.pageUrl !== elPageUrl) {
          if (!existing.pages) existing.pages = [];
          const pageEntry = { url: elPageUrl, title: elPageTitle };
        ...
        // New element — prefer per-element pageUrl over blanket context
        const elPageUrl = el.pageUrl || context?.pageUrl || '';
        const elPageTitle = el.pageTitle || context?.pageTitle || '';
        db.elements[el.name] = {
          widgetName: el.name,
          ...
          context: {
            pageUrl: elPageUrl,
            pageTitle: elPageTitle,
            parentDialog: context?.parentDialog || null,
          },
          pages: elPageUrl ? [{ url: elPageUrl, title: elPageTitle }] : [],
   ...
   * @returns {object} — Updated element DB
   */
  function enrichFromSteps(elementDB, steps, defaultPageUrl) {
    const db = elementDB || { elements: {} };
    if (!db.elements) db.elements = {};

    // Track current page context from Navigate steps
    let currentPage = defaultPageUrl ? { url: defaultPageUrl, title: '' } : null;

    for (const step of steps) {
      // Track page context from Navigate actions
      if (step.action === 'Navigate' && step.value) {
        currentPage = { url: step.value, title: '' };
        continue;
      }
      ...
      if (existing) {
        ...
        if (currentPage) {
          if (!existing.pages) existing.pages = [];
          if (!existing.pages.some(p => p.url === currentPage.url)) {
        ...
        db.elements[elementKey] = {
          ...
          selectors,
          context: currentPage ? { pageUrl: currentPage.url, pageTitle: currentPage.title } : {},
