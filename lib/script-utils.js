...
(function (exports) {
  ...
  const STEP_ACTION_CONFIG = {
    ...
    ClickListViewRow: { selector: false, value: true,  valuePlaceholder: "Row text (e.g. Current)" },
    ClickFirstDataGridRow: { selector: false, value: false },
    WaitForMicroflow: { selector: false, value: false },
    Screenshot:       { selector: false, value: true,  valuePlaceholder: "Filename (no extension)" },
    Capture:          { selector: true,  value: true,  selectorPlaceholder: "mx:widgetName or CSS selector", valuePlaceholder: "Variable name (e.g. ticketId)" },
  ...
  /**
   * Detect dynamically generated record IDs that should not be used as stable selectors.
   * Extends looksLikeGuid() to also catch patterns like "DB00000772", "INV-12345", "ORD00001".
   * These are short alpha prefixes (1-5 letters) followed by many digits (5+), optionally
   * separated by a dash or underscore.
   */
  function looksLikeDynamicId(value) {
    if (!value || typeof value !== 'string') return false;
    if (looksLikeGuid(value)) return true;
    const v = value.trim();
    // Alpha prefix (1-5 letters) + optional separator + 5+ digits (e.g. DB00000772, INV-12345)
    if (/^[A-Za-z]{1,5}[-_]?\d{5,}$/.test(v)) return true;
    return false;
  }
  ...
  /**
   * Resolve a step value that may contain {{varName}} variable references.
   * Returns { expr, isDynamic } where expr is the JS expression to embed in generated code.
   *  - Plain string:     { expr: "'hello'", isDynamic: false }
   *  - Exact variable:   { expr: "ticketId", isDynamic: true }
   *  - Mixed template:   { expr: "`Ticket: ${ticketId}`", isDynamic: true }
   */
  function resolveValue(rawValue) {
    if (!rawValue) return { expr: "''", isDynamic: false };
    // Exact match: {{varName}}
    const exactMatch = rawValue.match(/^\{\{(\w+)\}\}$/);
    if (exactMatch) return { expr: exactMatch[1], isDynamic: true };
    // Contains one or more {{var}} references — template literal
    if (/\{\{\w+\}\}/.test(rawValue)) {
      const tmpl = rawValue
        .replace(/\\/g, '\\\\')
        .replace(/`/g, '\\`')
        .replace(/\{\{(\w+)\}\}/g, (_, v) => '${' + v + '}');
      return { expr: '`' + tmpl + '`', isDynamic: true };
    }
    // Plain string
    return { expr: "'" + escapeJsString(rawValue) + "'", isDynamic: false };
  }
  ...
  function generateStepCode(step) {
    ...
    const SELECTOR_REQUIRED = ['Click', 'Fill', 'SelectDropdown', 'AssertText', 'AssertVisible', 'AssertEnabled', 'AssertDisabled', 'Capture'];
    ...
    const rv = resolveValue(step.value);

    let code;
    switch (step.action) {
      case "Navigate":
        code = `  await page.goto(${rv.expr});`; break;
      case "Login": {
      ...
      case "Fill":
        if (step.selector?.startsWith("mx:"))
          code = rv.isDynamic
            ? `  await mx.fillWidget(page, '${widgetName(step.selector)}', ${rv.expr});`
            : `  await mx.fillWidget(page, '${widgetName(step.selector)}', ${rv.expr});`;
        else
          code = `  await ${resolveLocator(step.selector)}.fill(${rv.expr});`;
        break;
      case "SelectDropdown":
        if (step.selector?.startsWith("mx:"))
          code = `  await mx.selectDropdown(page, '${widgetName(step.selector)}', ${rv.expr});`;
        else
          code = `  await mx.smartSelect(page, ${resolveLocator(step.selector)}, ${rv.expr});`;
        break;
      case "AssertText":
        if (step.selector?.startsWith("mx:"))
          code = `  await mx.assertWidgetText(page, '${widgetName(step.selector)}', ${rv.expr}, { soft: true });`;
        else
          code = `  await expect.soft(${resolveLocator(step.selector)}, 'Step ${step.order}: "${sel}" should contain "${val}"').toContainText(${rv.expr});`;
        break;
      case "Capture": {
        const varName = step.value?.trim() || 'capturedValue';
        if (!/^\w+$/.test(varName)) {
          throw new Error(`Step ${(step.order ?? 0) + 1} ("Capture"): Variable name "${varName}" must contain only letters, numbers, and underscores.`);
        }
        const loc = step.selector?.startsWith("mx:")
          ? `page.locator('.mx-name-${widgetName(step.selector)}')`
          : resolveLocator(step.selector);
        code = `  const ${varName} = (await ${loc}.textContent()).trim();`;
        break;
      }
      case "AssertVisible":
        if (step.selector?.startsWith("mx:"))
      ...
      case "PickDate": {
        ...
        const [pdY, pdM, pdD] = (step.value || '').split('-').map(Number);
        const yearArg = pdY ? `, ${pdY}` : '';
        code = `  await mx.pickDate(page, page.getByRole('button', { name: 'Show date picker' }).first(), ${pdD || 1}, ${pdM || 1}${yearArg});`;
        break;
      }
      case "Wait":
      ...
      case "ClickListViewRow":
        code = `  await mx.clickListViewRow(page, ${rv.expr});`; break;
      case "ClickFirstDataGridRow":
        code = `  await mx.clickDataGridFirstRow(page);`; break;
  ...
  function parseStatement(stmt) {
    ...
    // ── Capture (variable assignment from page text) ──
    m = stmt.match(/const\s+(\w+)\s*=\s*\(?\s*await\s+(.*?)\.textContent\(\)\s*\)?\.trim\(\)/);
    if (m) {
      const varName = m[1];
      const locatorExpr = m[2];
      const mxMatch = locatorExpr.match(/page\.locator\s*\(\s*['"]\.mx-name-(\w+)['"]/);
      const labelMatch = locatorExpr.match(/page\.getByLabel\s*\(\s*['"]([^'"]+)['"]/);
      const textMatch = locatorExpr.match(/page\.getByText\s*\(\s*['"]([^'"]+)['"]/);
      const placeholderMatch = locatorExpr.match(/page\.getByPlaceholder\s*\(\s*['"]([^'"]+)['"]/);
      const roleMatch = locatorExpr.match(/page\.getByRole\s*\(\s*['"]([^'"]+)['"]\s*,\s*\{\s*name:\s*['"]([^'"]+)['"]/);
      const cssMatch = locatorExpr.match(/page\.locator\s*\(\s*['"]([^'"]+)['"]/);
      let selector = '';
      if (mxMatch) selector = 'mx:' + mxMatch[1];
      else if (labelMatch) selector = 'label:' + labelMatch[1];
      else if (textMatch) selector = 'text:' + textMatch[1];
      else if (placeholderMatch) selector = 'placeholder:' + placeholderMatch[1];
      else if (roleMatch) selector = roleMatch[1] + ':' + roleMatch[2];
      else if (cssMatch) selector = selectorToMx(cssMatch[1]);
      return { action: 'Capture', selector, value: varName };
    }
    ...
    m = stmt.match(/mx\.clickListViewRow\s*\(\s*page\s*,\s*['"]([^'"]+)['"]/);
    if (m) return { action: 'ClickListViewRow', value: m[1] };
    if (/mx\.clickDataGridFirstRow\s*\(/.test(stmt)) return { action: 'ClickFirstDataGridRow' };
    if (/mx\.waitForMicroflow\s*\(/.test(stmt)) return { action: 'WaitForMicroflow' };

    // mx.pickDate(page, triggerLocator, day, month[, year])
    // The trigger locator may contain commas, so we match from the end: day, month[, year])
    m = stmt.match(/mx\.pickDate\s*\(.*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/);
    if (!m) m = stmt.match(/mx\.pickDate\s*\(.*,\s*(\d+)\s*,\s*(\d+)\s*\)/);
    if (m) {
      const d = m[1].padStart(2, '0');
      const mo = m[2].padStart(2, '0');
      const y = m[3] || new Date().getFullYear().toString();
      return { action: 'PickDate', value: `${y}-${mo}-${d}` };
    }
    ...
    // mx.fillWidget with variable reference (no quotes around value)
    m = stmt.match(/mx\.fillWidget\s*\(\s*page\s*,\s*['"]([^'"]+)['"]\s*,\s*(\w+)\s*\)/);
    if (m) return { action: 'Fill', selector: 'mx:' + m[1], value: '{{' + m[2] + '}}' };
    // mx.fillWidget with template literal
    m = stmt.match(/mx\.fillWidget\s*\(\s*page\s*,\s*['"]([^'"]+)['"]\s*,\s*`([^`]*)`/);
    if (m) return { action: 'Fill', selector: 'mx:' + m[1], value: m[2].replace(/\$\{(\w+)\}/g, '{{$1}}') };

    m = stmt.match(/mx\.selectDropdown\s*\(\s*page\s*,\s*['"]([^'"]+)['"]\s*,\s*['"]([^'"]*)['"]/);
    if (m) return { action: 'SelectDropdown', selector: 'mx:' + m[1], value: m[2] };
    // mx.selectDropdown with variable reference
    m = stmt.match(/mx\.selectDropdown\s*\(\s*page\s*,\s*['"]([^'"]+)['"]\s*,\s*(\w+)\s*\)/);
    if (m) return { action: 'SelectDropdown', selector: 'mx:' + m[1], value: '{{' + m[2] + '}}' };

    // mx.smartSelect with various locator types
    m = stmt.match(/mx\.smartSelect\s*\(\s*page\s*,\s*page\.getByLabel\s*\(\s*['"]([^'"]+)['"]\s*\)\s*,\s*['"]([^'"]*)['"]/);
    if (m) return { action: 'SelectDropdown', selector: 'label:' + m[1], value: m[2] };

    m = stmt.match(/mx\.smartSelect\s*\(\s*page\s*,\s*page\.getByRole\s*\(\s*['"]([^'"]+)['"]\s*,\s*\{\s*name:\s*['"]([^'"]+)['"][^}]*\}\s*\)\s*,\s*['"]([^'"]*)['"]/);
    if (m) return { action: 'SelectDropdown', selector: m[1] + ':' + m[2], value: m[3] };

    m = stmt.match(/mx\.smartSelect\s*\(\s*page\s*,\s*page\.getByText\s*\(\s*['"]([^'"]+)['"]\s*\)\s*,\s*['"]([^'"]*)['"]/);
    if (m) return { action: 'SelectDropdown', selector: 'text:' + m[1], value: m[2] };

    m = stmt.match(/mx\.smartSelect\s*\(\s*page\s*,\s*page\.getByPlaceholder\s*\(\s*['"]([^'"]+)['"]\s*\)\s*,\s*['"]([^'"]*)['"]/);
    if (m) return { action: 'SelectDropdown', selector: 'placeholder:' + m[1], value: m[2] };

    m = stmt.match(/mx\.smartSelect\s*\(\s*page\s*,\s*page\.locator\s*\(\s*['"]([^'"]+)['"]\s*\)\s*,\s*['"]([^'"]*)['"]/);
    ...
    // ── getByRole('gridcell'/'row', ...).click → ClickFirstDataGridRow ──
    // DataGrid cell/row clicks should always map to ClickFirstDataGridRow,
    // regardless of the cell content.
    if (/page\.getByRole\s*\(\s*['"](?:gridcell|row)['"]/.test(stmt) && /\.click\s*\(/.test(stmt)) {
      return { action: 'ClickFirstDataGridRow' };
    }
    ...
    // ── Variable-reference fills: .fill(varName) or .fill(`text ${var}`) ──
    // Locator-based bare variable fill
    m = stmt.match(/(page\.locator\s*\(\s*['"]([^'"]+)['"]\s*\)(?:\.[^.]+\s*\(\s*\))*)\s*\.fill\s*\(\s*(\w+)\s*\)/);
    if (m) return { action: 'Fill', selector: selectorToMx(m[2]), value: '{{' + m[3] + '}}' };
    // Locator-based template literal fill
    m = stmt.match(/(page\.locator\s*\(\s*['"]([^'"]+)['"]\s*\)(?:\.[^.]+\s*\(\s*\))*)\s*\.fill\s*\(\s*`([^`]*)`\s*\)/);
    if (m) return { action: 'Fill', selector: selectorToMx(m[2]), value: m[3].replace(/\$\{(\w+)\}/g, '{{$1}}') };
    // getByLabel/getByPlaceholder bare variable fill
    m = stmt.match(/page\.getBy(?:Label|Placeholder)\s*\(\s*['"]([^'"]+)['"]\s*\)(?:\s*\.\w+\s*\([^)]*\))*\s*\.fill\s*\(\s*(\w+)\s*\)/);
    if (m) {
      const prefix = /getByLabel/.test(stmt) ? 'label:' : 'placeholder:';
      return { action: 'Fill', selector: prefix + m[1], value: '{{' + m[2] + '}}' };
    }
  ...
  function cleanupScript(script) {
    ...
    // Count all occurrences of each sourceText in the full entries list
    ...
    const changes = entries
      ...
      .map(e => {
        ...
        return { original: e.stmt.text, reason: e.reason, occurrence: entryOccurrences[i] };
      });
  ...
  // ── Script splitting for "Record from here" ────────────

  /**
   * Split a script at a given step boundary for "Record from here" functionality.
   *
   * Returns { prefixStatements, suffixCode, splitLineIndex } where:
   * - prefixStatements: array of statement text strings for steps 0..stepIndex-1
   *   (to be replayed before recording)
   * - suffixCode: the portion of the script body AFTER the split point
   *   (will be replaced by the new recording)
   * - splitLineIndex: the line index in the test body where the split occurs
   *
   * If stepIndex is 0, prefixStatements is empty (record from the very beginning).
   * If stepIndex >= total steps, all statements are prefix (append mode).
   */
  function splitScriptAtStep(script, stepIndex) {
    if (!script) return { prefixStatements: [], suffixCode: '', splitLineIndex: 0 };

    const body = extractTestBody(script);
    if (!body) return { prefixStatements: [], suffixCode: '', splitLineIndex: 0 };

    const steps = parseScriptToSteps(script);
    if (!steps.length) return { prefixStatements: [], suffixCode: '', splitLineIndex: 0 };

    // Clamp stepIndex
    const idx = Math.max(0, Math.min(stepIndex, steps.length));

    // Collect the raw statements for all steps before the split point.
    // We use the sourceText from parsed steps which maps to actual script lines.
    const prefixStatements = [];
    for (let i = 0; i < idx; i++) {
      prefixStatements.push(steps[i].sourceText);
    }

    // Determine where in the body text the suffix starts.
    // Steps have startLine/endLine relative to the body text.
    const bodyLines = body.split('\n');
    let splitLineIndex;
    if (idx < steps.length) {
      // Split right before the selected step
      splitLineIndex = steps[idx].startLine;
    } else {
      // After all steps — suffix is empty
      splitLineIndex = bodyLines.length;
    }

    const suffixCode = bodyLines.slice(splitLineIndex).join('\n').trim();

    return { prefixStatements, suffixCode, splitLineIndex };
  }

  /**
   * Merge newly recorded code into an existing script, replacing everything
   * from stepIndex onward with the new recording.
   *
   * @param {string} originalScript - The full original script
   * @param {number} stepIndex - The step index where recording started
   * @param {string} newCode - The newly recorded code (raw body statements only)
   * @returns {string} The merged script
   */
  function mergeRecordedCode(originalScript, stepIndex, newCode) {
    if (!originalScript) return newCode;

    const body = extractTestBody(originalScript);
    if (!body) return originalScript;

    const steps = parseScriptToSteps(originalScript);
    const idx = Math.max(0, Math.min(stepIndex, steps.length));
    ...
    // Build the new body: prefix lines + new code
    const prefixLines = bodyLines.slice(0, splitLineIndex);
    // Ensure clean join: remove trailing empty lines from prefix
    while (prefixLines.length && prefixLines[prefixLines.length - 1].trim() === '') {
      prefixLines.pop();
    }

    const newBody = prefixLines.length
      ? prefixLines.join('\n') + '\n\n  ' + newCode.trim()
      : '  ' + newCode.trim();

    // Replace the body in the original script
    const scriptText = originalScript;
    const bodyStart = scriptText.indexOf(body);
    if (bodyStart === -1) return originalScript;

    return scriptText.substring(0, bodyStart) + newBody + scriptText.substring(bodyStart + body.length);
  }

  // ── Step Reorder & Insert ────────────────────────────────

  /**
   * Reorder a step in the script by moving it from one index to another.
   * Operates on the script text (source of truth) by extracting the test body,
   * rearranging the statement lines, and reconstructing the script.
   *
   * @param {string} script - The full script
   * @param {number} fromIndex - The step index to move
   * @param {number} toIndex - The target step index
   * @returns {string} The updated script
   */
  function reorderStepInScript(script, fromIndex, toIndex) {
    if (fromIndex === toIndex) return script;
    if (!script) return script;

    const body = extractTestBody(script);
    if (!body) return script;

    const steps = parseScriptToSteps(script);
    if (fromIndex < 0 || fromIndex >= steps.length) return script;
    if (toIndex < 0 || toIndex >= steps.length) return script;

    // Re-parse statements from the body to get accurate line ranges
    const stmts = splitIntoStatements(body);
    if (stmts.length !== steps.length) {
      // Mismatch — steps may have been filtered; fall back to rebuild approach
      return _reorderByRebuild(script, body, steps, fromIndex, toIndex);
    }

    const bodyLines = body.split('\n');

    // Build line-range segments: each step owns [startLine..endLine]
    // Plus any gap lines between steps (blank lines, comments)
    const segments = [];
    for (let i = 0; i < stmts.length; i++) {
      const start = stmts[i].startLine;
      const end = stmts[i].endLine;
      // Include trailing blank lines up to the next statement
      let trailEnd = end;
      const nextStart = (i + 1 < stmts.length) ? stmts[i + 1].startLine : bodyLines.length;
      // Include blank lines between this statement and the next
      for (let l = end + 1; l < nextStart; l++) {
        if (bodyLines[l].trim() === '') trailEnd = l;
        else break;
      }
      segments.push({ start, end: trailEnd, lines: bodyLines.slice(start, trailEnd + 1) });
    }
    ...
    // Any leading lines before the first statement (variable declarations, etc.)
    const firstStmtStart = stmts[0].startLine;
    const leadingLines = bodyLines.slice(0, firstStmtStart);

    // Reconstruct body
    const newBodyLines = [...leadingLines];
    for (const seg of segments) {
      newBodyLines.push(...seg.lines);
    }
    const newBody = newBodyLines.join('\n');

    // Replace body in the original script
    const bodyStart = script.indexOf(body);
    if (bodyStart === -1) return script;
    return script.substring(0, bodyStart) + newBody + script.substring(bodyStart + body.length);
  }

  /**
   * Fallback reorder: rebuild the body from step sourceTexts in the new order.
   */
  function _reorderByRebuild(script, body, steps, fromIndex, toIndex) {
    const ordered = steps.map(s => s.sourceText);
    const moved = ordered.splice(fromIndex, 1)[0];
    ordered.splice(toIndex, 0, moved);

    const indent = '  ';
    const newBody = ordered.map(src => {
      return src.split('\n').map(l => {
        const trimmed = l.trimStart();
        return trimmed ? indent + trimmed : l;
      }).join('\n');
  ...
  /**
   * Insert a new step into the script at a given index.
   * Generates code for the step and splices it into the test body.
   *
   * @param {string} script - The full script
   * @param {number} atIndex - The index to insert at (0 = before first step, steps.length = after last)
   * @param {object} step - Step object { action, selector?, value? }
   * @returns {string} The updated script
   */
  function insertStepInScript(script, atIndex, step) {
    if (!script) return script;
    ...
    const steps = parseScriptToSteps(script);
    const idx = Math.max(0, Math.min(atIndex, steps.length));

    let newCode;
    try {
      newCode = generateStepCode(step).trim();
    } catch {
      // Fallback for steps missing required fields (e.g. empty selector) —
      // insert a placeholder click that the user will fill in via the UI
      newCode = "await page.locator('').click();";
    }
    const bodyLines = body.split('\n');

    // Detect standard indentation from existing lines
    let indent = '  ';
    for (const line of bodyLines) {
      const m = line.match(/^(\s+)await /);
    ...
    let insertAt;
    if (idx >= steps.length) {
      // Append after last step — find the end of the last statement
      if (steps.length > 0) {
        const stmts = splitIntoStatements(body);
        const lastStmt = stmts[stmts.length - 1];
        insertAt = lastStmt ? lastStmt.endLine + 1 : bodyLines.length;
      } else {
        insertAt = bodyLines.length;
      }
    } else {
      // Insert before the step at idx
      const stmts = splitIntoStatements(body);
      // Map parsed steps back to statements by matching sourceText
      // Steps may be filtered subset of statements, so find the right statement
      let stmtIdx = 0;
      for (let si = 0; si < idx; si++) {
        // Advance stmtIdx to find the matching statement for each step
        while (stmtIdx < stmts.length && stmts[stmtIdx].text.trim() !== steps[si].sourceText.trim()) {
          stmtIdx++;
        }
        stmtIdx++;
      }
      // Now find the statement matching steps[idx]
      while (stmtIdx < stmts.length && stmts[stmtIdx].text.trim() !== steps[idx].sourceText.trim()) {
        stmtIdx++;
      }
      insertAt = stmtIdx < stmts.length ? stmts[stmtIdx].startLine : bodyLines.length;
    }

    // Insert the new code line(s) with a blank line separator
    const newLines = indentedCode.split('\n');
    bodyLines.splice(insertAt, 0, ...newLines);
  ...
  /**
   * Build a complete test script from an array of step objects.
   * Each step is passed through generateStepCode() to produce Playwright code.
   * Returns a script string ready for wrapScript().
   */
  function buildScriptFromSteps(steps, testName) {
    const stepLines = steps.map((step, idx) => {
      step.order = idx;
      return generateStepCode(step);
    });
    return `test('${escapeJsString(testName || 'Generated Test')}', async ({ page }) => {\n` +
  ...
  exports.looksLikeDynamicId = looksLikeDynamicId;
  exports.cleanupScript = cleanupScript;
  exports.splitScriptAtStep = splitScriptAtStep;
  exports.mergeRecordedCode = mergeRecordedCode;
  exports.reorderStepInScript = reorderStepInScript;
  exports.insertStepInScript = insertStepInScript;
  exports.buildScriptFromSteps = buildScriptFromSteps;
